...
class Prompt(BertPreTrainedModel):
    ...
    def __init__(self, config, graph_type='GAT', layer=1, path_list=None, data_path=None, depth2label=None, seloss_wight=None, **kwargs):
        ...
        if self.data_path.split('/')[-1] == 'WebOfScience':
            with open(os.path.join(self.data_path, 'wos.taxnomy'), 'r') as f:
                for line in f.readlines():
                    line = line.strip().split('\t')
                    taxnomy.append(line)
